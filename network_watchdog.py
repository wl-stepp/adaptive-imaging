...
if __name__ == "__main__":
    ...
    positions = getTilePositions_v2(np.ones((inputSize, inputSize)),
                                    nnImageSize)
    setWidth = 0.5
...
def on_modified(event):
    ...
    file = open(os.path.join(os.path.dirname(model_path), 'binary_output.dat'),
                mode='rb')
    content = file.read()
    file.close()
    ...
    if frameNum >= len(content)-1:
        ...
        print(int((frameNum-1)/2), ' passed because newer file already there')
        return
    ...
    if size > mitoSize*0.95:
        ...
        # If this is the first frame, get the parameters for contrast and
        # prepare figure
        inputSize = mitoFull.shape[0]
        print(frameNum)
        if frameNum == 1 and not inputSize == inputSizeOld:
            inputSize = round(drpFull.shape[0]*resizeParam) if not \
            ...
            setColor = [0.7, 0.7, 0.7]
            for axes in ax:
                lines.append(Axes.axhline(
                    axes, y=inputSize - positions['stitch'],
                    linewidth=setWidth, color=setColor))
                lines.append(Axes.axvline(
                    axes, x=inputSize - positions['stitch'],
                    linewidth=setWidth, color=setColor))
                for x in positions['px']:
                    lines.append(Axes.axhline(
                        axes, y=x[0] + positions['stitch'],
                        linewidth=setWidth, color=setColor))
                    lines.append(Axes.axvline(
                        axes, x=x[1] + positions['stitch'],
                        linewidth=setWidth, color=setColor))
        ...
        for p in positions['px']:
            outputDataFull[p[0]+stitch:p[2]-stitch, p[1]+stitch:p[3]-stitch] =\
                           output_predict[i, stitch:stitch1, stitch:stitch1, 0]
            mitoDataFull[p[0]+stitch:p[2]-stitch, p[1]+stitch:p[3]-stitch] = \
                inputData[i, stitch:stitch1, stitch:stitch1, 0]
            drpDataFull[p[0]+stitch:p[2]-stitch, p[1]+stitch:p[3]-stitch] = \
        ...
        # alphas =  exposure.rescale_intensity(
        # mitoDataFull,(filters.threshold_mean(mitoDataFull),
