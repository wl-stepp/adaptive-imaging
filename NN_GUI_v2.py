...
from PyQt5.QtWidgets import QWidget, QSlider, QPushButton, QLabel, QGridLayout,\
                            QFileDialog, QProgressBar, QGroupBox, QApplication
...
class MultiPageTIFFViewerQt(QWidget):
    ...
    def __init__(self,app):
        ...
        self.viewer_mito.rightMouseButtonDoubleClicked.connect(self.mouseDouble)
        ...
        self.outputPlot.addItem(self.frameLine, ignoreBounds=True)
    ...
    def loadData(self):
        ...
        for frame in range(0, self.image_mitoOrig.shape[0]):
            inputData, positions = prepareNNImages(self.image_mitoOrig[frame,
                                                                       :, :], 
                                                self.image_drpOrig[frame, :, :], 
                                                    nnImageSize)
            ...
            for position in positions['px']:
                nnOutput[frame, position[0]+stitch:position[2]-stitch,
                            position[1]+stitch:position[3]-stitch] = \
                            output_predict[i,stitch:stitch1,stitch:stitch1,0]
                mitoDataFull[frame, position[0]+stitch:position[2]-stitch,
                           position[1]+stitch:position[3]-stitch] = \
                           inputData[i,stitch:stitch1,stitch:stitch1,0]
                drpDataFull[frame, position[0]+stitch:position[2]-stitch,
                           position[1]+stitch:position[3]-stitch] = \
    ...
    def onTimer(self):
        ...
        qimage_mitoOrig = qimage2ndarray.array2qimage(self.image_mitoOrig
                                                      [i, :, :],normalize=True)
        qimage_drpOrig = qimage2ndarray.array2qimage(self.image_drpOrig
                                                     [i, :, :], normalize=True)
        qimage_drp = qimage2ndarray.array2qimage(self.image_drp[i, :, :],
                                                 normalize=False)
        qimage_mito = qimage2ndarray.array2qimage(self.image_mito[i, :, :],
                                                  normalize=False)
        qimage_nn = qimage2ndarray.array2qimage(self.image_nn[i, :, :],
                                                normalize=(0, 50))
    ...
    def mouseRelease(self, x, y):
        ...
        for remoteViewer in [self.viewer_drp, self.viewer_nn]:
            if remoteViewer.canZoom:
                viewBBox = remoteViewer.zoomStack[-1] if len(remoteViewer.zoomStack) else remoteViewer.sceneRect()
                selectionBBox = self.viewer_mito.scene.selectionArea().boundingRect().intersected(viewBBox)
